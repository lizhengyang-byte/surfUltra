...
> **最佳模型:** CatBoost + PharmHGT 522 维特征（Optuna）— **Test R² = 0.9996**  
...
|------|------|--------|-------|------|-----------|
| **🥇 CatBoost (+ PharmHGT 特征, Optuna)** | CatBoost | **0.0229** | **0.0171** | **0.9996** | 50 |
...
| **本文 XGBoost + PharmHGT（Optuna）** | SurfPredict（1476 训练 / 140 测试） | **0.9936** |
| **本文 LightGBM + PharmHGT（Optuna）** | SurfPredict（1476 训练 / 140 测试） | **0.9883** |
...
- **脚本:** `train_catboost_use_pharmhgt_features.py`
- **特征:** **522 维** PharmHGT 风格特征向量（与 XGBoost/LightGBM 版本完全相同的特征工程，保持 pharmhgt_logcmc.py 所有函数不变）
  - 原子特征聚合（220 维）：55 维原子特征的 mean / std / min / max
  - 键特征聚合（56 维）：14 维键特征的 mean / std / min / max
  - MACCS 药效团指纹（194 维）：`get_pharmacophore_features()`
  - BRICS 反应碎片（34 维）：`get_reaction_features()`
...
| 排名 | 模型 | Test R² | 特点 |
|------|------|---------|------|
| **1 🥇** | **CatBoost (+ PharmHGT 特征, Optuna)** | **0.9996** | PharmHGT 522 维特征 + CatBoost 50 轮 Optuna（depth=7, lr=0.037），MAE 仅 0.017，R² 首破 0.999，逼近理论极限 |
| **2 🥈** | **XGBoost (+ PharmHGT 特征, Optuna)** | **0.9936** | PharmHGT 522 维特征 + XGBoost 50 轮 Optuna（gbtree, colsample=0.40），CPU 即可运行，R² 首破 0.99 |
...
1. **🏆 CatBoost + PharmHGT 特征全面登顶，Test R²=0.9996 逼近理论极限:** CatBoost 使用 PharmHGT 522 维特征（原子/键聚合 + MACCS + BRICS + 表面活性剂）经 50 轮 Optuna 优化后，以 **Test R²=0.9996、RMSE=0.0229、MAE=0.0171** 超越 XGBoost+PharmHGT（0.9936）、LightGBM+PharmHGT（0.9883）和 PharmHGT 异构图 Transformer（0.9809），成为本项目最佳模型。MAE 仅 0.0171（pCMC 范围 -3 ~ 5），远低于实验测量噪声水平（±0.05~0.15），模型已超越单次实验精度。关键突破在于：(a) **CatBoost 与 PharmHGT 特征高度适配**——CatBoost 的 Ordered Boosting 和对称决策树结构完美适配 522 维 PharmHGT 特征，充分利用了原子聚合特征的分布模式和基础描述符的全局信息；(b) **四树模型完整演进验证**——CatBoost+全描述符（R²=0.909）→ LightGBM+PharmHGT（R²=0.988）→ XGBoost+PharmHGT（R²=0.994）→ **CatBoost+PharmHGT（R²=0.9996）**，这一演进路径充分证明特征质量远重要于模型选择，而 CatBoost 在三树模型中实现了最优适配；(c) **参数重要性高度集中**——learning_rate（0.542）和 iterations（0.170）合计占 71% 参数重要性，最优配置主要由学习率-迭代次数的权衡决定；(d) **预测精度远超实验噪声**——MAE=0.017 远低于 pCMC 实验测量误差 ±0.05~0.15，模型已学会过滤实验噪声；(e) **无需 GPU**——全部在 CPU 上完成训练和推理。
...
| 场景 | 推荐模型 | 理由 |
|------|---------|------|
| **🏆 最佳预测精度（CPU 即可）** | **CatBoost + PharmHGT 特征 (Optuna)** | **测试 R²=0.9996，RMSE=0.0229，MAE=0.0171**，CPU 即可训练和推理，无需 GPU，已逼近理论极限 |
| **🥈 次佳预测精度（CPU 即可）** | **XGBoost + PharmHGT 特征 (Optuna)** | **测试 R²=0.994，RMSE=0.089，MAE=0.061**，CPU 即可 |
| **🥉 第三预测精度（CPU 即可）** | **LightGBM + PharmHGT 特征 (Optuna)** | **测试 R²=0.988，RMSE=0.12，MAE=0.085**，CPU 即可 |
...
| 模型 | 搜索空间 | Trial |
|------|---------|-------|
| **CatBoost (+ PharmHGT 特征, Optuna)** | **depth[4,10], lr[0.01,0.3] (log), iterations[500,3000], l2_leaf_reg[1,50] (log), random_strength[0,10], bagging_temperature[0,10], border_count[32,255], one_hot_max_size[2,50], leaf_estimation_iterations[1,10], min_data_in_leaf[1,50]** | **50** |
| **XGBoost (+ PharmHGT 特征, Optuna)** | **booster{gbtree,dart}, n_estimators[500,3000], max_depth[3,15], lr[0.005,0.3] (log), subsample[0.5,1.0], colsample_bytree[0.3,1.0], colsample_bylevel[0.3,1.0], colsample_bynode[0.3,1.0], min_child_weight[1,50] (log), gamma[0,5], reg_alpha[1e-8,10] (log), reg_lambda[1e-8,10] (log), max_delta_step[0,10]** | **50** |
