...
| ③ | PharmHGT 风格特征 | **522** | 各 `*_pharmhgt_features.py` / `build_feature_vector(smi)` | **R²=0.9996** |
...
graph TD
    ...
    J --> N[XGBoost R²=0.9936]
    J --> O[LightGBM R²=0.9883]
    D --> P[CatBoost R²=0.909]
    D --> Q[LightGBM R²=0.899]
    D --> R[MLP R²=0.865]
...
| PharmHGT 522 维 | CatBoost, XGBoost, LightGBM | **0.988~0.9996** |
...
```text
性能阶梯（R² ↑）：
  0.40-0.63   │  线性模型 / 62维基线
  0.79-0.84   │  SMILES序列模型 / 精选描述符 MLP
  0.83-0.87   │  全描述符 MLP / XGBoost / RNN
  0.89-0.91   │  全描述符 树模型（CatBoost/LightGBM） ★ 传统方法天花板
  0.98-0.99   │  PharmHGT 特征 + 树模型（LightGBM/XGBoost）
  0.9996      │  PharmHGT 特征 + CatBoost ★ 当前最佳
```
...
各 `*_pharmhgt_features.py`（CatBoost/XGBoost/LightGBM 各一份，代码自包含），以及 `pharmhgt_logcmc.py`（PharmHGT 异构图 Transformer 原始实现），进而封装为 `models/predictor/featurizer.py`。
...
| 模型 | 测试 R² | 测试 RMSE | 测试 MAE | 训练/推理 |
|:----:|:-------:|:---------:|:--------:|:---------:|
| CatBoost (Optuna) | **0.9996** | 0.0229 | 0.0171 | CPU |
| XGBoost (Optuna) | **0.9936** | 0.0890 | 0.0609 | CPU |
| LightGBM (Optuna) | **0.9883** | 0.1200 | 0.0853 | CPU |
| PharmHGT Transformer | **0.9809** | 0.1534 | 0.1189 | GPU |
...
|:----:|------|:------:|----------|
| 1 | **LogP** | 1273 | 脂溶性最关键 |
| 2 | **tail_ratio** | 1254 | 疏水尾链占比（领域特征！）|
| 3 | **MolWt** | 1119 | 分子量 |
| 4 | **atom_mean_35** | 715 | 原子平均质量 |
| 5 | **atom_std_25** | 704 | 原子隐式氢数标准差 |

**分析**: LightGBM 的特征重要性呈现明显的"两极化"——LogP 和 tail_ratio 占绝对主导，说明 LightGBM 对 522 维特征的利用更偏向少数强预测特征。
...
| **H 供体/受体** | ✅ 显式编码 | ❌ 未包含 |
| **模型** | 树模型 (CatBoost/XGBoost/LightGBM) | AttentiveFP (GNN) |
| **状态** | ✅ 已完成 | ⏳ 待运行 |
...
```
SMILES 字符串
  → SMILES_TOKEN_REGEX 正则分词
    → token 序列: ['C', 'C', 'C', 'C', '=', 'O', ...]
  → Word2Vec (CBOW/Skip-gram) 训练
    → 每个 token 映射为 embedding_dim 维向量
  → 所有 token 向量 → mean pooling → 分子特征向量
```
...
| 特征 | CatBoost+PharmHGT | XGBoost+PharmHGT | LightGBM+PharmHGT | 化学意义 |
|------|:-----------------:|:-----------------:|:-----------------:|----------|
| **LogP** | 🥇 7.1 | 🥈 0.025 | 🥇 1273 | 疏水性 — CMC 核心决定因素 |
| **tail_ratio** | 🥉 2.6 | 🅃 0.018 | 🥈 1254 | 疏水尾链占比 — 领域特征 |
| **MolWt** | 🥉 4.7 | — | 🥉 1119 | 分子量 |
| **NAtoms/HeavyAtoms** | 🥈 6.0/4.6 | — | — | 分子大小 |
| **atom_std_46** (Gasteiger σ) | — | 🥇 0.041 | 🅃 704 (atom_std_25) | 电荷分布均匀性 |
...
1. **LogP 是跨模型的通用最强特征**：在三棵树模型的特征重要性 Top 5 中均出现，且稳定列前茅。这与表面活性剂化学一致——亲水-亲油平衡直接决定胶束化倾向。

2. **tail_ratio 是领域特化特征的成功案例**：PharmHGT 独有的"尾链占比"特征在 LightGBM 中排名第 2（重要值 1254），在 CatBoost 中排名第 5（重要值 2.6）。这是通用特征方案（RDKit 描述符）无法提供的。
...
特征方案演进：
  ...
  522 维 PharmHGT      → 最佳 R² ~0.9996（CatBoost）
...
**结论**：在分子性质预测中，**特征工程投入的回报远高于模型调参**。从 62→217→522 维特征，R² 从 0.84→0.91→0.9996，每一步的增益远超同特征下切换模型架构的增益。
...
其中，tail_ratio 在 LightGBM 中排名第 2（重要值 1254），证明**领域知识注入**是 PharmHGT 特征成功的关键。
...
| `train_catboost_use_pharmhgt_features.py` | PharmHGT 522 维 | `build_feature_vector(smi)` | 自包含（CatBoost） |
| `train_lightgbm_use_pharmhgt_features.py` | PharmHGT 522 维 | `build_feature_vector(smi)` | 自包含（LightGBM） |
| `train_xgboost_use_pharmhgt_features.py` | PharmHGT 522 维 | `build_feature_vector(smi)` | 自包含（XGBoost） |
| `models/predictor/featurizer.py` | PharmaHGT 522 + RDKit 217 | `build_feature_vector_pharmhgt(smi)`, `smiles_to_features_all(smi)` | 统一封装供 `predictor_single.py` 等脚本使用 |
...
| 场景 | 推荐特征方案 | 理由 |
|:----:|:-----------:|------|
| 🏆 **最佳精度** | **PharmHGT 522 维** | R²=0.9996，CatBoost，CPU 可运行 |
...
| 🚫 **无 GPU 场景** | **PharmHGT 522 + 树模型** | 全 CPU，R²>0.988 |
