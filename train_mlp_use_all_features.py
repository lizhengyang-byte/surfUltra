...
def train_model(lr, dropout, wd, hidden, bs, max_epochs=500):
    ...
    model = MLP(X_train_sub_s.shape[1], hidden, dropout).to(device)
...
model = MLP(X_full_s.shape[1], best_params["hidden"], best_params["dropout"]).to(device)
...
with torch.no_grad():
    ...
    print(f"{'MLP (Train)':<25} {train_rmse:>8.4f} {train_mae:>8.4f} {train_r2:>8.4f}")
    ...
    print(f"{'MLP (Test)':<25} {test_rmse:>8.4f} {test_mae:>8.4f} {test_r2:>8.4f}")

print("-" * 52)
print(f"\n✅ 全部 RDKit 描述符 ({X_full_s.shape[1]} 维) + PyTorch MLP")
...
print("-" * 52)
print(f"{'SVR (原最佳)':<25} {'0.6826':>8} {'0.4436':>8} {'0.6227':>8}")
print(f"{'MLP (本模型)':<25} {test_rmse:>8.4f} {test_mae:>8.4f} {test_r2:>8.4f}")